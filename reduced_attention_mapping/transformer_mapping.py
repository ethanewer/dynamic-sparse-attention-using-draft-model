...
from transformers import LlamaModel, LlamaConfig  # type: ignore
...
class LlamaEncoder(nn.Module):
    def __init__(
        ...
        self.model = LlamaModel(config).to(device, dtype)

    def forward(self, x: Tensor) -> Tensor:
        _, batch_size, _, seq_len = x.shape

        x /= x.sum(dim=-1)[..., None]

        x = rearrange(x, "l b h t -> b t (l h)")
...
#         else:
#             self.q_proj = nn.Linear(input_size, hidden_dim, device=device, dtype=dtype)
#             self.k_proj = nn.Linear(input_size, hidden_dim, device=device, dtype=dtype)
...
#             key = key[:, None]
#             value = value[:, None]
#             attn_output = F.scaled_dot_product_attention(query, key, value)[:, 0]
...
class TransformerAttentionMapping(AttentionMapping):
    ...
    def __init__(
        ...
        # hidden_dim: int = 512,
        # seq_len_as_hidden_dim: bool = False,
